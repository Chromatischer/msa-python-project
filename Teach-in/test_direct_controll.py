...
if __name__ == "__main__":
    ...
    while True:
        if txsticki.getPos("left", "X") != 0:
            txtim.MovementAgent.DirectControl.move_m1(txsticki.getPos("left", "X"))
        else:
            stxtc.txt.motor(1).stop()

        if txsticki.getPos("left", "Y") != 0:
            txtim.MovementAgent.DirectControl.move_m2(txsticki.getPos("left", "Y"))
        else:
            stxtc.txt.motor(2).stop()

        if txsticki.getPos("right", "X") != 0:
            print(txsticki.getPos("right", "X"))
            txtim.MovementAgent.DirectControl.move_m3(txsticki.getPos("right", "X"))
        else:
            stxtc.txt.motor(3).stop()

        if txsticki.getPos("right", "Y") != 0:
            txtim.MovementAgent.DirectControl.move_m4(txsticki.getPos("right", "Y"))
        else:
